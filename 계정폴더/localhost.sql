...
(SELECT STUDENT_NAME
FROM ACADEMY_B
WHERE STUDENT_NO = 1003 OR STUDENT_NO = 1004);  --에러 발생
-- = 대신 IN으로 바꾸면 실행이 된다
-- 이유는 =으로는 조건이 2가지 이상 실행이 안되기 때문이다

SELECT STUDENT_NAME
FROM ACADEMY_B
WHERE STUDENT_NO = 1003 OR STUDENT_NO = 1004;   --에러 발생 안함
...
SELECT TYPE, COUNT(NAME) AS COUNT   --COUNT(NAME)은 중복을 제거하고 갯수를 나타낸다 INDEXOF처럼
FROM GIFT   --GIFT 테이블에서 실행
GROUP BY TYPE   --TYPE을 하나로 묶는다
HAVING COUNT(NAME) >= 2 --HAVING COUNT로 숫자가 2이상 인것들만 출력(중복 TYPE이 2개 이상인 것들만 출력)
ORDER BY TYPE ASC;
...
SELECT STUDENT_NAME, SCORE, RANK() OVER(ORDER BY SCORE ASC)   --DESC는 내림차순
FROM STUDENT_GRADE;

SELECT STUDENT_NAME, SCORE, RANK(SELECT SCORE FROM STUDENT_GRADE ORDER BY SCORE DESC) 
FROM STUDENT_GRADE; --RANK()는 무조건 중복없는 순위가 매겨짐

SELECT STUDENT_NAME, SCORE, DENSE_RANK() OVER(ORDER BY SCORE DESC)
FROM STUDENT_GRADE; --DENSE_RANK()는 중복 있는 순위가 매겨짐

SELECT STUDENT_NAME, SCORE, ROW_NUMBER() OVER(ORDER BY SCORE DESC)
FROM STUDENT_GRADE; --점수가 같으면 이름별로 판별하여 순위가 바뀜

SELECT TYPE, SUM(PRICE) AS SUM FROM GIFT GROUP BY TYPE;
--GIFT 테이블의 TYPE을 그룹으로 묶고 SUM으로 TYPE별로 PRICE를 더했다

SELECT TYPE, PRICE, SUM(PRICE) OVER(PARTITION BY TYPE) AS SUM FROM GIFT;
--PARTITION은 TYPE을 나눈다

SELECT TYPE, MAX(PRICE) AS MAX FROM GIFT GROUP BY TYPE;
--MAX는 그룹핑 TYPE 중에 가장 PRICE가 큰 것을 출력

SELECT TYPE, MIN(PRICE) AS MIN FROM GIFT GROUP BY TYPE;
--MIN은 그룹핑 TYPE 중에 가장 PRICE가 작은 것을 출력

SELECT TYPE, AVG(PRICE) AS AVG FROM GIFT GROUP BY TYPE;
--AVG는 그룹핑 TYPE 중에 중복 TYPE을 다 더한 값의 평균을 출력

---------------------------------------------------

CREATE OR REPLACE VIEW ACADEMY_ALL AS
SELECT * FROM ACADEMY_A;    --VIEW 생성

INSERT INTO ACADEMY_ALL VALUES(1005, '김경범');
...
INSERT INTO ACADEMY_STUDENT_NAME VALUES('1001');
...
SELECT EMP_SEQ2.CURRVAL --현재 시퀀스 값
FROM DUAL;
...
ALTER SEQUENCE EMP_SEQ INCREMENT BY 1;  --4씩 증가
